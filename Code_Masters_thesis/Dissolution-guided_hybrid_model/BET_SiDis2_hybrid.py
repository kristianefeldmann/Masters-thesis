...
# To allow for capturing curvature, if only linear it could only model a straight line. Account for non-linear relationships between input and output
...
# Predictions for the original literature data points only
a_pred_lit = model_a.predict(si_al_poly)
p_pred_lit = model_p.predict(si_al_poly)
m_pred_lit = model_m.predict(si_al_poly)
kd_pred_lit = model_kd.predict(si_al_poly)

# Calculate R^2 values
r2_a = r2_score(a_lit, a_pred_lit)
r2_p = r2_score(p_lit, p_pred_lit)
r2_m = r2_score(m_lit, m_pred_lit)
r2_kd = r2_score(kd_lit, kd_pred_lit)
...
class HybridNet(nn.Module):
    ...
    def forward(self, x):
        ...
        si_al = x[:, 0]
        BET = x[:, 1]
        zeoConc = x[:, 8] * 1000  # g/mL -> g/dm^3
        t = x[:, 7]

        si_al_std = si_al.detach().cpu().numpy().reshape(-1, 1)
        ...
        A = zeoConc * BET
        beta = kd_user_trans_poly / si_al
        C_si_t = beta * A * si_al * t
